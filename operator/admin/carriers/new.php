...
$sPath = "/etc/asterisk/sip-register/"; $sURI = $sPath . $_VARS['input_carrierhost'] . "-" . $_VARS['input_username'];
	...
	tech='SIP', path='".$_VARS['input_carrierhost']."-".$_VARS['input_username']."', prefix='".$_VARS['input_prefix']."'");
	//reload sip server
	...
	echo "<SCRIPT LANGUAGE='JavaScript'>window.location.replace('./new_carrier.php')</script>";
	}
	else
	{
...
echo'<form action="'.$_SERVER['PHP_SELF'].'" method="post">
			...
			printFormText("PREFIX if needed or leave empty" ,"input_prefix",$_VARS['input_prefix']);
