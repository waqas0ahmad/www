...
IF ($_POST['vads_amount']<50) {
        $_POST['theAmount']="50";
    }
    if (_POST["vads_amount"] !=''){
        $_POST['theAmount']=$_POST["vads_amount"];
    }
...
$sTexteLibre = "DIRECT=>".$_POST['vads_cust_name'];
$args = array(
    ...
    "vads_trans_date" => gmdate('YmdHis'),
    "vads_cust_last_name" => $_POST['vads_cust_name']
);
$hostconf = "localhost";
...
echo '<div class="span11" style="background-color:white;padding: 10px;border-radius: 10px;">
                <div class="span4"><img src="http://151.80.45.192/interface/admin/img/cic.jpg"  width="100"/>
                <img src="http://151.80.45.192/interface/admin/img/credit-mutuel.jpg"  width="100"/>
                <img src="http://151.80.45.192/interface/admin/img/monetico-paiement.jpg" width="100"></div>
            </div>
...
$form = '<form action="'.$formData['form']['action'].'" method="'.$formData['form']['method'].'" accept-charset="'.$formData['form']['accept-charset'].'">';
  foreach ($formData['fields'] as $name => $value) {
    ...
    $form .= '<input readonly="readonly" type="hidden" name="'.$name.'" value="'.$value.'" />';
   }
