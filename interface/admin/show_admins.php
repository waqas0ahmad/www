	...
	switch ($_VARS['action'])
		...
		case "add":     
  			...
  			if ($_VARS['button'] == "send" AND sizeof($reval) == 0)
    				...
    				$sInputSQL = "INSERT INTO ".ASTCC.".webadmins SET admin_username='".clean($_VARS['input_webuser'])."', admin_password='$input_md5webpw',
					...
					admin_hausnr='".$_VARS['input_hausnr']."', admin_plz='".$_VARS['input_plz']."', admin_ort='".$_VARS['input_ort']."', 
					admin_gebdat='".$_VARS['input_gebdat']."',
					admin_email='".$_VARS['input_email']."', admin_telefon='".$_VARS['input_telefon']."', admin_ip='".$REMOTE_ADDR."', 
					lastlogin=NOW(), created=NOW()";
...
?>
				...
				<?=printFormSelect(translate("adminstatus") ,"input_adminstatus", array("Administrator" => 0, "Superadministrator" => 1 ),$_VARS['input_adminstatus']);?>
		...
		case "del":
			...
			$sCountQuery = "SELECT * FROM ".ASTCC.".webadmins";
			...
			if($aPager->total_pages!=1)
			{
			  ...
			  for($i=$aPager->offset_page;$i<$aPager->last_page_in_pageList;$i++)
			  {
				...
				{
				  $sHeadline.="<td background='../imgs/pager/pager.gif' width='20' height='20' align='center'><a href=\"show_admins.php?iPage=$i&title=" . translate("admineditheadline") . "\" class='pager'>".($i+1)."</a></td> ";
				}
			  }
		...
		case "edit":
			# Check form if necessary fields are filled out 
  			...
  			if ($_VARS['button'] == "send" AND sizeof($reval) == 0)
  			{
				...
				if (strlen($_VARS['input_webpw']) > 1)
				{
					...
					$sInputSQL = "UPDATE ".ASTCC.".webadmins SET admin_username='".$_VARS['input_webuser']."', admin_password='$input_md5webpw',
						admin_status='".$_VARS['input_adminstatus']."', admin_vorname='".$_VARS['input_vorname']."', admin_nachname='".$_VARS['input_nachname']."',
						admin_firma='".$_VARS['input_firma']."', admin_strasse='".$_VARS['input_strasse']."', admin_adresszusatz='".$_VARS['input_adresszusatz']."',
			...
			$aSelectedAdmin = $objAstcc->select("SELECT * FROM ".ASTCC.".webadmins WHERE admin_id='" . $_VARS['id'] . "'");
			?>			
  ...
  echo '<form action="http://151.80.45.192/interface/admin/uploadpub.php" method="post" enctype="multipart/form-data">
    Pub a afficher:
    <input type="file" name="fileToUpload" id="fileToUpload">
    <input type="submit" value="Telecharger" name="submit">
</form>';
  echo '<form action="http://151.80.45.192/interface/admin/uploadtexte.php" method="post" enctype="multipart/form-data">
    Texte a afficher:
    <input type="text" name="textTochange" id="textTochange" value="L EQUIPE JKCALL VOUS SOUHAITE UNE BONNE FAITE DE L AID">
